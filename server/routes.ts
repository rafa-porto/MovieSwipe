...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  router.get("/movies", async (req: Request, res: Response) => {
    try {
      ...
      if (req.query.streamingServices) {
        filters.streamingServices = (req.query.streamingServices as string).split(',');
      }
    ...
    try {
      ...
      if (action === 'liked') {
        ...
        if (!(userPrefs.liked_movies as number[]).includes(movieId)) {
          const likedMovies = [...(userPrefs.liked_movies as number[]), movieId];
        ...
        if (!(userPrefs.disliked_movies as number[]).includes(movieId)) {
          const dislikedMovies = [...(userPrefs.disliked_movies as number[]), movieId];
          await storage.updateUserPreferences(userId, {
  ...
  router.get("/users/:id/stats", async (req: Request, res: Response) => {
    try {
      ...
      const likeRate = totalMovies > 0 ? Math.round((likedMovies / totalMovies) * 100) : 0;
      ...
      const sortedGenres = Object.entries(likedGenres)
        ...
        .slice(0, 3)
        .map(([genre, count]) => {
          const percentage = totalMovies > 0 ? Math.round((count / totalMovies) * 100) : 0;
          return { genre, count, percentage };
        });
      
      // Get streaming platform counts
      const streamingServices = preferences.streaming_services as Record<string, number>;
  ...
  router.get("/users/:id/recommendations", async (req: Request, res: Response) => {
    try {
      ...
      const recommendations = await storage.getRecommendedMovies(userId, limit);
