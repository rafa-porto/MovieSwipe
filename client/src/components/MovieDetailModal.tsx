...
const MovieDetailModal = ({ movie, onClose }: MovieDetailModalProps) => {
  ...
  // Simular a obtenção de detalhes extras do filme
  useEffect(() => {
    const fetchMovieDetails = async () => {
      setLoading(true);
      try {
        // Aqui você poderia fazer uma chamada API para obter mais detalhes
        // Por enquanto, apenas adicionamos alguns dados fictícios
        setDetails({
          ...movie,
          cast: [
            'Ator Principal', 
    ...
    >
      <div className="relative bg-[#1E293B] rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div 
          ...
          style={{ 
            backgroundImage: `url(https://image.tmdb.org/t/p/w1280${details.backdrop_path || details.poster_path})` 
          }}
        >
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {activeTab === 'overview' && (
            <div>
              <h3 className="text-lg font-poppins font-semibold text-[#EAEAEA] mb-2">Sinopse</h3>
              <p className="text-[#EAEAEA]/80 mb-6 leading-relaxed">
                {details.overview}
              </p>
              
              <h3 className="text-lg font-poppins font-semibold text-[#EAEAEA] mb-2">Direção</h3>
              <p className="text-[#EAEAEA]/80 mb-6">
                {details.director}
              </p>
              
              <h3 className="text-lg font-poppins font-semibold text-[#EAEAEA] mb-2">Disponível em</h3>
          ...
          {activeTab === 'cast' && (
            <div>
              <h3 className="text-lg font-poppins font-semibold text-[#EAEAEA] mb-4">Elenco Principal</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {details.cast?.map((actor, index) => (
                  <div key={index} className="p-4 bg-[#121826]/70 backdrop-blur-sm rounded-lg flex items-center">
                    ...
                    <div>
                      <p className="font-medium text-[#EAEAEA]">{actor}</p>
                      <p className="text-xs text-[#EAEAEA]/60">Personagem {index + 1}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'trailer' && (
            <div>
              <h3 className="text-lg font-poppins font-semibold text-[#EAEAEA] mb-4">Trailer Oficial</h3>
                  ...
                  <div className="text-center p-4">
                    ...
                    <p className="text-[#EAEAEA]/80">
                      ...
                      <span className="text-sm text-[#EAEAEA]/60">
                        Tente novamente mais tarde ou acesse o site oficial do filme.
                      </span>
                    </p>
                  </div>
                </div>
